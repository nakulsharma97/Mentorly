...
export default function RoleSwitcher({ profile, onProfileUpdated }) {
  ...
  const handleRoleSwitch = async (newRole) => {
    try {
      ...
      setMessage(
        `Successfully switched to ${newRole === "MENTOR" ? "Mentor" : "Learner"} role!`,
      );
      ...
      setTimeout(() => {
        setSwitching(false);
        navigate(
          newRole === "MENTOR" ? "/mentor/dashboard" : "/learner/dashboard",
          {
            replace: true,
          },
        );
      ...
      setMessage(
        `Error switching role: ${error.response?.data?.message || error.message}`,
      );
      setSwitching(false);
    }
  };
        ...
        <div className="mb-4">
          <p className="text-sm text-on-surface-variant mb-3">
            ...
            <span className="font-bold text-on-surface">
              {currentRole === "MENTOR" ? "👨‍🏫 Mentor" : "👨‍🎓 Learner"}
            </span>
          </p>
          <p className="text-xs text-on-surface-variant">
            Want to experience the platform from a different perspective? Switch
            roles to see both dashboards.
          </p>
        </div>
