...
const SESSION_TEMPLATE_PRESETS = [
  {
    ...
    values: {
      ...
      description:
        "Hands-on architecture mentoring for engineers who want stronger system design decisions, trade-off thinking, and scalable implementation patterns.",
      sessionType: "1:1 Mentoring",
      priceAmount: "79",
      maxParticipants: 1,
    },
  },
  {
    ...
    values: {
      ...
      description:
        "Focused mock interview with targeted feedback on communication, coding approach, and problem-solving speed for your next role.",
  ...
  },
  {
    ...
    values: {
      ...
      description:
        "Interactive small-group lab with practical exercises, peer learning, and mentor guidance to improve delivery confidence and consistency.",
...
export default function TeachingPage({ notify }) {
  ...
  const validateSessionForm = (form) => {
    ...
    if (
      !Number.isFinite(cancellationWindowHours) ||
      cancellationWindowHours < 0
    ) {
      errors.cancellationWindowHours =
        "Cancellation window must be 0 or greater.";
    }
  ...
  const updateSessionField = (field, value) => {
    setSessionForm((prev) => {
      ...
      setSessionErrors((current) =>
        Object.keys(current).length > 0 ? validateSessionForm(next) : current,
      );
      return next;
    });
  };
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const failedCalls = responses.filter(
      ...
      setSessionForm((prev) => ({
        ...
        ...Object.fromEntries(
          Object.entries(parsed).filter(([key]) =>
            Object.prototype.hasOwnProperty.call(INITIAL_SESSION_FORM, key),
          ),
        ),
      }));
  ...
  const upcomingSessions = useMemo(() => {
    ...
    const allUpcoming = mentorSessions
      .filter(
        (session) =>
          new Date(session?.startTime || 0).getTime() >= now - 10 * 60 * 1000,
      )
      .sort(
        (a, b) =>
    ...
    () =>
      roadmaps
        ...
        .map((roadmap) => ({
          ...
          progress: Math.min(
            100,
            Math.max(0, Number(roadmap?.progressPercent || 0)),
          ),
        })),
    [mentorId, roadmaps],
  );
  ...
  const monthlyGrowth = useMemo(() => {
    ...
    return mentorReleasedPayments
      .filter(
        (payment) => new Date(payment?.createdAt || 0).getTime() >= cutoff,
      )
  ...
  const activeLearners = useMemo(
    () =>
      new Set(
        mentorBookings.map((booking) => booking?.learner?.id).filter(Boolean),
      ).size,
    [mentorBookings],
  );
  ...
  const handleBookingAction = async (bookingId, status) => {
    ...
    try {
      ...
      setActionMessage(
        getInfoFeedback("bookingUpdatedStatus", { bookingId, status }).message,
      );
      notify?.({
        type: "success",
    ...
    try {
      const response = await client.post("/api/v1/sessions", {
        ...
        endTime: sessionForm.endTime
          ? new Date(sessionForm.endTime).toISOString()
          : null,
      ...
      notify?.({
        ...
        message:
          "Your session is live and visible to learners in Manage Sessions.",
      ...
      setSessionMessage(
        error?.response?.data?.data?.error || "Could not create session.",
      );
      notify?.({
        type: "error",
  ...
  const handleAddAvailabilitySlot = async (event) => {
    ...
    if (!availabilityForm.timezone) {
      setAvailabilityMessage("Please provide a valid timezone.");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-surface text-on-surface pb-24 md:pb-0">
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pt-8 pb-10 space-y-12">
        <section
          id="overview"
          className="rounded-2xl border border-outline-variant/15 bg-surface-container-low p-5 sm:p-6 lg:p-8 shadow-sm"
        >
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.24em] text-on-tertiary-fixed-variant">
                Mentor Hub
              </p>
              <h1 className="mt-2 text-3xl font-extrabold tracking-tight text-on-surface sm:text-4xl">
                Manage your sessions from one place
              </h1>
              <p className="mt-2 max-w-2xl text-sm leading-relaxed text-on-surface-variant sm:text-base">
                Create a session, publish availability, and keep your teaching
                workspace organized.
              </p>
            </div>
        ...
        <section
          id="sessions"
          className={`rounded-2xl border border-outline-variant/15 bg-surface-container-low p-5 sm:p-6 lg:p-8 shadow-sm space-y-6 ${currentWorkspaceTab !== "sessions" ? "opacity-80" : ""}`}
        >
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className="text-2xl font-bold text-on-surface">
                Publish a new teaching slot
              </h2>
            </div>
            <div className="flex flex-col items-start gap-2 sm:items-end">
              <p className="text-sm text-on-surface-variant">
                Visible in your mentor hub and ready for learners to book.
              </p>
              {draftSavedLabel && (
              ...
              <label className="inline-flex items-center gap-2 rounded-full border border-outline-variant/20 bg-surface-container-lowest px-3 py-1.5 text-xs font-semibold text-on-surface">
                ...
                <span>
                  {publishSoundEnabled
                    ? "Publish sound on"
                    : "Publish sound off (default muted)"}
                </span>
              </label>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-6 xl:grid-cols-5">
            <form
              className="grid grid-cols-1 gap-4 rounded-2xl border border-outline-variant/15 bg-surface-container-lowest p-4 sm:p-5 xl:col-span-3 lg:grid-cols-2"
            ...
            >
              <label className="grid gap-2">
                ...
                <input
                  required
                  className={`rounded-lg border bg-surface-container-lowest px-4 py-3 ${sessionErrors.title ? "border-red-400" : "border-outline-variant/20"}`}
                  value={sessionForm.title}
                  onChange={(e) => updateSessionField("title", e.target.value)}
                  placeholder="React Architecture Clinic"
                />
                {sessionErrors.title && (
              ...
              <label className="grid gap-2">
                ...
                <input
                  required
                  className={`rounded-lg border bg-surface-container-lowest px-4 py-3 ${sessionErrors.sessionType ? "border-red-400" : "border-outline-variant/20"}`}
                  value={sessionForm.sessionType}
                  onChange={(e) =>
                    updateSessionField("sessionType", e.target.value)
                  }
                  placeholder="1:1 Mentoring"
                />
              ...
              <label className="grid gap-2 lg:col-span-2">
                ...
                <textarea
                  required
                  rows={4}
                  className={`rounded-lg border bg-surface-container-lowest px-4 py-3 ${sessionErrors.description ? "border-red-400" : "border-outline-variant/20"}`}
                  value={sessionForm.description}
                  onChange={(e) =>
                    updateSessionField("description", e.target.value)
                  }
                  placeholder="Describe the outcome, audience, and what learners should prepare."
                />
              ...
              <label className="grid gap-2">
                ...
                <input
                  required
                  type="datetime-local"
                  className={`rounded-lg border bg-surface-container-lowest px-4 py-3 ${sessionErrors.startTime ? "border-red-400" : "border-outline-variant/20"}`}
                  value={sessionForm.startTime}
                  onChange={(e) =>
                    updateSessionField("startTime", e.target.value)
                  }
                />
              ...
              <label className="grid gap-2">
                ...
                <input
                  required
                  type="datetime-local"
                  className={`rounded-lg border bg-surface-container-lowest px-4 py-3 ${sessionErrors.endTime ? "border-red-400" : "border-outline-variant/20"}`}
                  value={sessionForm.endTime}
                  onChange={(e) =>
                    updateSessionField("endTime", e.target.value)
                  }
                />
              ...
              <label className="grid gap-2">
                ...
                <input
                  ...
                  min="0"
                  step="0.01"
                  className={`rounded-lg border bg-surface-container-lowest px-4 py-3 ${sessionErrors.priceAmount ? "border-red-400" : "border-outline-variant/20"}`}
                  value={sessionForm.priceAmount}
                  onChange={(e) =>
                    updateSessionField("priceAmount", e.target.value)
                  }
                  placeholder="50"
                />
              ...
              <label className="grid gap-2">
                ...
                <input
                  className={`rounded-lg border bg-surface-container-lowest px-4 py-3 ${sessionErrors.meetingLink ? "border-red-400" : "border-outline-variant/20"}`}
                  value={sessionForm.meetingLink}
                  onChange={(e) =>
                    updateSessionField("meetingLink", e.target.value)
                  }
                  placeholder="https://meet.google.com/..."
                />
                {sessionErrors.meetingLink && (
                  <span className="text-xs font-semibold text-red-600">
                    {sessionErrors.meetingLink}
                  </span>
                )}
              ...
              <label className="grid gap-2">
                ...
                <input
                  type="number"
                  min="0"
                  className={`rounded-lg border bg-surface-container-lowest px-4 py-3 ${sessionErrors.cancellationWindowHours ? "border-red-400" : "border-outline-variant/20"}`}
                  value={sessionForm.cancellationWindowHours}
                  onChange={(e) =>
                    updateSessionField(
                ...
                />
                {sessionErrors.cancellationWindowHours && (
                  <span className="text-xs font-semibold text-red-600">
                    {sessionErrors.cancellationWindowHours}
                  </span>
                )}
              ...
              <label className="grid gap-2">
                ...
                <input
                  type="number"
                  min="0"
                  className={`rounded-lg border bg-surface-container-lowest px-4 py-3 ${sessionErrors.rescheduleWindowHours ? "border-red-400" : "border-outline-variant/20"}`}
                  value={sessionForm.rescheduleWindowHours}
                  onChange={(e) =>
                    updateSessionField("rescheduleWindowHours", e.target.value)
                  }
                />
                {sessionErrors.rescheduleWindowHours && (
                  <span className="text-xs font-semibold text-red-600">
                    {sessionErrors.rescheduleWindowHours}
                  </span>
                )}
              ...
              <label className="grid gap-2">
                ...
                <input
                  type="number"
                  min="1"
                  className={`rounded-lg border bg-surface-container-lowest px-4 py-3 ${sessionErrors.maxParticipants ? "border-red-400" : "border-outline-variant/20"}`}
                  value={sessionForm.maxParticipants}
                  onChange={(e) =>
                    updateSessionField("maxParticipants", e.target.value)
                  }
                />
              ...
              <div className="flex items-end lg:col-span-2">
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    type="submit"
                    className="rounded-lg bg-primary px-5 py-3 text-sm font-bold text-on-primary hover:opacity-90"
                  >
                    Review and create session
                  </button>
                  <button
                    type="button"
                    className="rounded-lg border border-outline-variant/20 px-5 py-3 text-sm font-bold text-on-surface hover:bg-surface-container-high"
                    onClick={handleResetSessionForm}
                  >
                    Clear form
                  </button>
                </div>
            ...
            <aside className="space-y-4 rounded-2xl border border-outline-variant/15 bg-surface-container-lowest p-4 sm:p-5 xl:col-span-2">
              <div className="flex items-center justify-between">
                ...
                <span className="rounded-full bg-primary/10 px-3 py-1 text-xs font-bold text-primary">
                  {mentorSessionCards.length}
                </span>
              </div>
              {mentorSessionCards.length === 0 ? (
                <p className="text-sm text-on-surface-variant">
                  No sessions yet. Publish your first one using the form.
                </p>
              ) : (
                <div className="space-y-3">
                  {mentorSessionCards.map((item) => (
                    <article
                      key={item.id}
                      className="rounded-xl border border-outline-variant/15 bg-surface-container px-4 py-3"
                    >
        ...
        <section
          id="availability"
          className={`rounded-2xl border border-outline-variant/15 bg-surface-container-low p-5 sm:p-6 lg:p-8 shadow-sm space-y-6 ${currentWorkspaceTab !== "availability" ? "opacity-80" : ""}`}
        >
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.24em] text-on-tertiary-fixed-variant">
              ...
              <h2 className="text-2xl font-bold text-on-surface">
                Publish your teaching windows
              </h2>
            </div>
            <p className="text-sm text-on-surface-variant">
              This helps learners find a matching time slot before booking.
            </p>
          </div>
          {availabilityMessage && (
            <p className="text-sm font-semibold text-primary">
          ...
          >
            <label className="grid gap-2">
              ...
              <select
                className="rounded-lg border border-outline-variant/20 bg-surface-container-lowest px-4 py-3"
            ...
            <div className="flex items-end lg:col-span-4">
              <button
                type="submit"
                className="rounded-lg border border-outline-variant/20 bg-surface-container-lowest px-5 py-3 text-sm font-bold text-on-surface hover:bg-surface-container-high"
              >
                Add availability
              </button>
            </div>
          ...
          <div className="space-y-3">
            {availabilitySlots.length === 0 ? (
              <p className="text-sm text-on-surface-variant">
                No availability slots yet. Add one above so learners can book
                your sessions.
              </p>
            ) : (
              availabilitySlots.slice(0, 4).map((slot) => (
                <div
                  key={slot.id}
                  className="flex items-center justify-between gap-4 rounded-xl border border-outline-variant/10 bg-surface-container-high px-4 py-3"
                >
                  <div>
                  ...
                  <button
                    type="button"
                    className="rounded-lg border border-outline-variant/20 px-3 py-2 text-xs font-bold text-on-surface hover:bg-surface-container-low"
        ...
        <section
          id="verification"
          className={`rounded-2xl border border-outline-variant/15 bg-surface-container-low p-5 sm:p-6 lg:p-8 shadow-sm space-y-3 ${currentWorkspaceTab !== "verification" ? "opacity-80" : ""}`}
        >
          <p className="text-xs font-bold uppercase tracking-[0.24em] text-on-tertiary-fixed-variant">
            Verification
          </p>
          <h2 className="text-2xl font-bold text-on-surface">
            Keep your mentor verification active
          </h2>
          <p className="text-sm text-on-surface-variant max-w-3xl">
            If you clicked verification from the mentor hub, this section is
            where you can review the status and keep the rest of the workspace
          ...
          <>
            <section className="grid grid-cols-1 gap-8 lg:grid-cols-12 lg:items-end">
              <div className="lg:col-span-8">
                <p className="mb-2 block text-xs font-semibold uppercase tracking-wider text-on-tertiary-fixed-variant">
                  Instructor Overview
                </p>
                <h1 className="mb-6 text-4xl font-extrabold tracking-tight text-on-surface sm:text-5xl lg:text-7xl">
              ...
              <div className="lg:col-span-4">
                <div className="rounded-2xl bg-primary p-6 sm:p-8 text-on-primary shadow-lg">
                  <div className="mb-8 flex items-start justify-between gap-4">
                    <div>
                      <p className="text-sm font-medium text-primary-fixed/80">
                        Current Balance
                      </p>
            ...
            <section className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              <div className="flex h-full flex-col rounded-2xl border border-outline-variant/15 bg-surface-container-low p-5 sm:p-6">
                <div className="mb-6 flex items-center justify-between gap-4">
                  ...
                  <span className="rounded-full bg-tertiary-fixed px-3 py-1 text-xs font-bold text-on-tertiary-fixed">
                    {requestCards.length} NEW
                  </span>
                </div>
                  ...
                  {requestCards.length === 0 ? (
                    <div>
                      ...
                      <p className="mt-2 text-xs text-on-surface-variant">
                        Tip: keep your session slots active in the sessions tab
                        to receive requests.
                      </p>
                    </div>
                  ) : (
                    requestCards.map((request) => (
                      <div
                        key={request.id}
                        className="group rounded-lg bg-surface-container-lowest p-4 transition-all hover:shadow-md"
                      >
                        ...
                        <div className="mt-3 grid grid-cols-2 gap-2">
                          <button
                            className="rounded-lg bg-primary py-2 text-xs font-bold text-on-primary"
                            type="button"
                            onClick={() =>
                          ...
                          <button
                            className="rounded-lg bg-surface-container py-2 text-xs font-bold text-on-surface-variant"
                            type="button"
                            onClick={() =>
                  ...
                  {upcomingSessions.length === 0 ? (
                    ...
                    upcomingSessions.map((session) => (
                      <article key={session.id} className="relative">
                        <div
                          className={`absolute -left-[29px] top-1 h-4 w-4 rounded-full ${session.primary ? "bg-primary" : "bg-outline-variant"} ring-4 ring-surface-container-lowest`}
                        />
                        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                          <div>
                            <span className="text-xs font-bold uppercase tracking-widest text-on-tertiary-fixed-variant">
                              {session.time}
                            </span>
                  ...
                  <div>
                    ...
                    <div className="h-2 w-full overflow-hidden rounded-full bg-outline-variant/20">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, Math.max(0, avgCompletion))}%`,
                        }}
                      />
                    </div>
                  </div>
              ...
              <div className="rounded-2xl border border-outline-variant/15 bg-surface-container-highest/30 p-5 sm:p-6 md:col-span-1 lg:col-span-1 flex flex-col justify-center">
                ...
                <p className="mt-3 text-xs text-on-surface-variant">
                  Live count from accepted and pending mentor bookings.
                </p>
              </div>
            </section>
          </>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 px-4">
          <div className="w-full max-w-lg rounded-2xl border border-outline-variant/20 bg-surface-container-low p-5 shadow-2xl sm:p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-on-tertiary-fixed-variant">
                  Confirm Session
                </p>
                <h3 className="mt-2 text-xl font-bold text-on-surface">
                  Ready to publish this session?
                </h3>
              </div>
              <button
                type="button"
            ...
            <div className="mt-5 space-y-3 rounded-xl border border-outline-variant/15 bg-surface-container-lowest p-4 text-sm">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <span className="font-bold text-on-surface">
                  {formatTimeRange(sessionForm.startTime, sessionForm.endTime)}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-on-surface-variant">Price</span>
                <span className="font-bold text-on-surface">
                  {formatCurrency(sessionForm.priceAmount || 0)}
                </span>
              </div>
              <div className="flex items-center justify-between gap-3">
                <span className="text-on-surface-variant">Participants</span>
                <span className="font-bold text-on-surface">
                  {sessionForm.maxParticipants || 1}
                </span>
              </div>
            </div>
