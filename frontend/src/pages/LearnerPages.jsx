...
export function LearnerSkillsPage() {
  ...
  // ── keep the block closed ──
  // (replaces old exports from line 870 to 1033)

  const savedSkillNames = useMemo(
    () =>
      new Set(
        savedSkills.map((item) =>
          String(item?.skillName || item?.name || "").toLowerCase(),
        ),
      ),
    [savedSkills],
  );
  const mentorSkillCounts = useMemo(() => {
    ...
    mentors.forEach((mentor) => {
      splitSkills(mentor.skills).forEach((skill) => {
  ...
  const mentorSkillList = useMemo(
    () =>
      Array.from(mentorSkillCounts.keys()).sort(
        (a, b) => mentorSkillCounts.get(b) - mentorSkillCounts.get(a),
      ),
    [mentorSkillCounts],
  );
  ...
  async function toggleSkillWatch(skillName) {
    ...
    try {
      ...
      window.console.error(toggleError);
    }
  }
  ...
  return (
    <div className="lp-shell">
      <PageHeader
        ...
        subtitle="Actual skills from the backend skill catalog and mentor knowledge graph."
      />
      <Toolbar>
        <SearchField
          value={query}
      ...
      <div className="ld-stats md-animate">
        <StatsCard
          ...
          label="Skills"
          value={skills.length}
          description="Loaded from /api/v1/skills"
        />
        <StatsCard
          icon="groups"
          label="Mentor skills"
          value={mentorSkillList.length}
          description="Observed from mentor profiles"
        />
        <StatsCard
          icon="bookmark"
          label="Saved skills"
        ...
        <StatsCard
          icon="school"
          label="Mentors teaching"
          value={mentors.length}
          description="Current mentor roster"
        />
      </div>

      <SectionCard title="Skill catalog" icon="auto_stories">
        {loading ? (
          <LoadingBlock />
        ) : error ? (
          <ErrorBlock
          ...
          <div className="lp-grid lp-grid--skills">
            {filteredSkills.map((skill) => {
              const mentorCount =
                mentorSkillCounts.get(skill.name.toLowerCase()) || 0;
              const saved = savedSkillNames.has(skill.name.toLowerCase());
              return (
                <SkillCard
                  ...
                  saved={saved}
                  onToggle={() => toggleSkillWatch(skill.name)}
                />
              );
            })}
          </div>
          ...
          <EmptyState
            ...
            description="Try another skill or category from the backend catalog."
          />
        )}
      </SectionCard>

      <SectionCard title="Saved skills" icon="bookmark">
        {savedSkills.length ? (
          <div className="lp-chip-list">
            {savedSkills.map((item) => (
              <button
          ...
          <EmptyState
            ...
            description="Add skills to your watchlist to personalize mentor discovery."
          />
        )}
      </SectionCard>
    </div>
