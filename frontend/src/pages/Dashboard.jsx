...
export default function Dashboard({ onLogout, language, page = 'mentors', notify }) {
  ...
  const runMentorSearch = useCallback(async (queryOverride, filtersOrOptions = {}, maybeOptions = {}) => {
    const finalQuery = String(queryOverride ?? searchQuery ?? '').trim();
    const looksLikeFilters = filtersOrOptions && typeof filtersOrOptions === 'object' && (
      ...
      || Object.prototype.hasOwnProperty.call(filtersOrOptions, 'availability')
    );
    ...
    const loadRecommendations = async () => {
      ...
      try {
        ...
        const results = await Promise.all(
          queries.map((query) => client.get('/api/v1/search/mentors', { params: { keyword: query, size: 12 } }))
        );
            ...
            <div className="quick-search-chips">
              {recentSearches.map((item) => (
                <button
                  ...
                  onClick={() => {
                    setSearchQuery(item);
                    runMentorSearch(item, activeFilters, { skipRecent: true });
                  }}
                >
                  {item}
                </button>
