...
export default function Dashboard({
  ...
  const initialMentorSort = ALLOWED_MENTOR_SORTS.has(searchParams.get("sort"))
  ...
  const [mentorVerificationForm, setMentorVerificationForm] = useState({
    documentUrl: "",
  ...
  const [verificationWorkflowMessage, setVerificationWorkflowMessage] =
    useState("");
  const [mentorLearnerReviewForm, setMentorLearnerReviewForm] = useState({
    learnerId: "",
    bookingId: "",
    rating: "5",
    comment: "",
    ...
    const load = async () => {
      try {
        ...
        setSelectedBookingId(
          (current) =>
            current || (bookingList[0] ? String(bookingList[0].id) : ""),
        );
        ...
        const mentorCandidate = sessionList.find(
          (session) =>
            session.mentor?.id && session.mentor.id !== currentProfile?.id,
        );
        setOverlapMentorId(
          (current) =>
            current ||
            (mentorCandidate ? String(mentorCandidate.mentor.id) : ""),
        );

        setPaymentError(
          !currentProfile
            ? getErrorFeedback("dashboardReducedFeaturesProfilePending").message
            : "",
        );
      } catch (error) {
        ...
        setProfile({
          id: "error",
          fullName: "Unable to load",
          email: "error@skillswap",
        });
      } finally {
        setIsLoading(false);
      }
  ...
  useEffect(() => {
    ...
    loadHistory().catch(() => {
      if (isMounted) {
        setChatError(
          getErrorFeedback("dashboardChatHistoryLoadFailed").message,
        );
      }
    });

    const socket = new WebSocket(
      `${wsBase}/ws/chat?bookingId=${encodeURIComponent(selectedBookingId)}`,
    );
    ...
    socket.onmessage = (event) => {
      try {
        ...
        setChatError(
          getErrorFeedback("dashboardRealtimeMalformedMessage").message,
        );
      }
    };
    ...
    socket.onerror = () => {
      setChatError(
        getErrorFeedback("dashboardRealtimeConnectionError").message,
      );
    };
  ...
  }, [profile?.id, selectedBookingId, wsBase]);
    ...
    const loadTrustData = async () => {
      try {
        ...
        if (profile.role === "ADMIN") {
          const [moderationReportsRes, verificationQueueRes] =
            await Promise.all([
              client.get("/api/v1/safety/reports", {
                params: { moderationQueue: true, status: "OPEN" },
              }),
              client.get("/api/v1/verification/mentor/requests", {
  ...
  const handleBookingStatusUpdate = async (bookingId, status, message) => {
    ...
    try {
      ...
      setBookingActionMessage(
        backendError ||
          getErrorFeedback("dashboardBookingStatusUpdateFailed").message,
      );
    }
  };
  ...
  const handleVerificationSubmit = async (taskId) => {
    ...
    try {
      const response = await client.post(
        `/api/v1/verification/tasks/${taskId}/submit`,
        { submissionText },
      );
      setVerificationSubmissions((prev) => [response.data.data, ...prev]);
      setVerificationAnswers((prev) => ({ ...prev, [taskId]: "" }));
      setVerificationMessage(
        getInfoFeedback("verificationTaskSubmitted").message,
      );
    } catch {
      setVerificationMessage(
        getErrorFeedback("submitVerificationTaskFailed").message,
      );
    }
  };
  ...
  const handleReviewSubmission = async (submissionId, status) => {
    ...
    try {
      const response = await client.patch(
        `/api/v1/verification/submissions/${submissionId}`,
        { status },
      );
      setVerificationSubmissions((prev) =>
        prev.map((s) => (s.id === submissionId ? response.data.data : s)),
      );
      if (status === "APPROVED") {
      ...
      setVerificationMessage(
        getInfoFeedback("verificationSubmissionMarked", { status }).message,
      );
    } catch {
      setVerificationMessage(
        getErrorFeedback("reviewVerificationSubmissionFailed").message,
      );
    }
  };
  ...
  const handleRoadmapUpdate = async (roadmapId, progressPercent) => {
    try {
      ...
      setRoadmaps((prev) =>
        prev.map((roadmap) =>
          roadmap.id === roadmapId ? response.data.data : roadmap,
        ),
      );
    } catch {
      setVerificationMessage(
        getErrorFeedback("updateRoadmapProgressFailed").message,
      );
    }
  };
  ...
  const handleMarkNotificationRead = async (notificationId) => {
    ...
    try {
      ...
      setNotificationMessage(
        getErrorFeedback("markNotificationReadFailed").message,
      );
    }
  };
  ...
  const handleMarkAllNotificationsRead = async () => {
    ...
    try {
      ...
      setNotificationMessage(
        getErrorFeedback("markAllNotificationsReadFailed").message,
      );
    }
  };
  ...
  const handleSaveMentor = async (mentorId) => {
    ...
    try {
      const response = await client.post(
        `/api/v1/watchlist/mentors/${mentorId}`,
      );
      ...
      setWatchlistMessage(
        err?.response?.data?.data?.error ||
          getErrorFeedback("watchlistSaveMentorFailed").message,
      );
    }
  };
  ...
  const handleRemoveMentor = async (mentorId) => {
    ...
    try {
      ...
      setSavedMentors((prev) =>
        prev.filter((item) => item.mentor?.id !== mentorId),
      );
    } catch {
      setWatchlistMessage(
        getErrorFeedback("watchlistRemoveMentorFailed").message,
      );
    }
  };
  ...
  const handleAddWatchSkill = async () => {
    ...
    setWatchlistMessage("");
    try {
      const response = await client.post("/api/v1/watchlist/skills", {
      ...
      setWatchlistMessage(
        err?.response?.data?.data?.error ||
          getErrorFeedback("watchlistAddSkillFailed").message,
      );
    }
  };
  ...
  const handleRemoveWatchSkill = async (skillName) => {
    ...
    try {
      await client.delete(
        `/api/v1/watchlist/skills/${encodeURIComponent(skillName)}`,
      );
      setWatchedSkills((prev) =>
        prev.filter(
          (item) =>
            String(item.skillName).toLowerCase() !==
            String(skillName).toLowerCase(),
        ),
      );
    } catch {
      setWatchlistMessage(
        getErrorFeedback("watchlistRemoveSkillFailed").message,
      );
    }
  };
  ...
  const handleAddAvailabilitySlot = async (event) => {
    ...
    try {
      ...
      setAvailabilityMessage(
        err?.response?.data?.data?.error ||
          getErrorFeedback("availabilityAddSlotFailed").message,
      );
    }
  };
  ...
  const handleDeleteAvailabilitySlot = async (slotId) => {
    ...
    try {
      ...
      setAvailabilityMessage(
        getErrorFeedback("availabilityDeleteSlotFailed").message,
      );
    }
  };
  ...
  const handleFindOverlapSlots = async () => {
    ...
    try {
      ...
      setAvailabilityMessage(
        err?.response?.data?.data?.error ||
          getErrorFeedback("availabilityOverlapFetchFailed").message,
      );
    }
  };
  ...
  const handleRecommendOverlapSlots = async () => {
    ...
    try {
      const response = await client.get(
        "/api/v1/availability/recommendations",
        {
          params: {
            mentorId: Number(overlapMentorId),
            timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC",
      ...
      setAvailabilityMessage(
        err?.response?.data?.data?.error ||
          getErrorFeedback("availabilityRecommendationsFetchFailed").message,
      );
    }
  };
  ...
  const handleBlockUser = async (userId) => {
    setSafetyMessage("");
    try {
      const response = await client.post(`/api/v1/safety/block/${userId}`, {
      ...
      setSafetyMessage(
        err?.response?.data?.data?.error ||
          getErrorFeedback("safetyBlockUserFailed").message,
      );
    }
  };
  ...
  const handleSubmitReport = async (event) => {
    ...
    try {
      ...
      setSafetyMessage(
        err?.response?.data?.data?.error ||
          getErrorFeedback("safetySubmitReportFailed").message,
      );
    }
  };
  ...
  const handleEscalateReport = async (reportId) => {
    ...
    try {
      ...
      const moderationRes = await client.get("/api/v1/safety/reports", {
        params: { moderationQueue: true, status: "OPEN" },
      });
      ...
      setSafetyMessage(
        err?.response?.data?.data?.error ||
          getErrorFeedback("safetyEscalateReportFailed").message,
      );
    }
  };
  ...
  const handleSubmitMentorVerification = async (event) => {
    ...
    try {
      await client.post(
        "/api/v1/verification/mentor/request",
      ...
      setVerificationWorkflowMessage(
        getInfoFeedback("mentorVerificationSubmitted").message,
      );
      setMentorVerificationForm((prev) => ({ ...prev, documentUrl: "" }));
    } catch (err) {
      setVerificationWorkflowMessage(
        err?.response?.data?.data?.error ||
          getErrorFeedback("mentorVerificationSubmitFailed").message,
      );
    }
  };
  ...
  const handleReviewMentorVerification = async (requestId, status) => {
    ...
    try {
      ...
      const queueRes = await client.get(
        "/api/v1/verification/mentor/requests",
        { params: { status: "PENDING" } },
      );
      setMentorVerificationQueue(queueRes?.data?.data || []);
      setVerificationWorkflowMessage(
        getInfoFeedback("mentorVerificationMarked", { status }).message,
      );
    } catch (err) {
      setVerificationWorkflowMessage(
        err?.response?.data?.data?.error ||
          getErrorFeedback("mentorVerificationUpdateFailed").message,
      );
    }
  };
  ...
  const handleSubmitLearnerReview = async (event) => {
    ...
    try {
      ...
      setVerificationWorkflowMessage(
        getInfoFeedback("learnerReviewSubmitted").message,
      );
      setMentorLearnerReviewForm((prev) => ({ ...prev, comment: "" }));
    } catch (err) {
      setVerificationWorkflowMessage(
        err?.response?.data?.data?.error ||
          getErrorFeedback("learnerReviewSubmitFailed").message,
      );
    }
  };

  const runMentorSearch = useCallback(
    async (queryOverride, filtersOrOptions = {}, maybeOptions = {}) => {
      const finalQuery = String(queryOverride ?? searchQuery ?? "").trim();
      const looksLikeFilters =
        filtersOrOptions &&
        typeof filtersOrOptions === "object" &&
        (Object.prototype.hasOwnProperty.call(filtersOrOptions, "priceMin") ||
          Object.prototype.hasOwnProperty.call(filtersOrOptions, "priceMax") ||
          Object.prototype.hasOwnProperty.call(filtersOrOptions, "rating") ||
          Object.prototype.hasOwnProperty.call(filtersOrOptions, "level") ||
          Object.prototype.hasOwnProperty.call(
            filtersOrOptions,
            "availability",
          ));
      const filters = looksLikeFilters ? filtersOrOptions : {};
      const options = looksLikeFilters ? maybeOptions : filtersOrOptions;
      setMentorSearchLoading(true);
      setMentorSearchMessage("");
      try {
        const params = { q: finalQuery, size: 50 };
        if (Number(filters.priceMin || 0) > 0) {
          params.minPrice = filters.priceMin;
        }
        if (Number(filters.priceMax || 500) < 500) {
        ...
        const response = await client.get("/api/v1/search/mentors", { params });
  ...
  const handleBookNearestSession = async (mentorId) => {
    ...
    if (!profile?.id) {
      setMentorSearchMessage(
        getInfoFeedback("mentorSearchLoginToBook").message,
      );
      return;
    }

    try {
      const sessionsResponse = await client.get(
        `/api/v1/sessions/mentor/${mentorId}`,
      );
      const mentorSessions = sessionsResponse?.data?.data || [];
      if (mentorSessions.length === 0) {
        setMentorSearchMessage(
          getInfoFeedback("mentorSearchNoUpcoming").message,
        );
        return;
      }

      const nearestSession = [...mentorSessions].sort(
        (a, b) =>
      ...
      if (!nearestSession?.id) {
        setMentorSearchMessage(
          getErrorFeedback("mentorSearchNoValidSession").message,
        );
        return;
      }

      const bookingResponse = await client.post("/api/v1/bookings", {
      ...
      setMentorSearchMessage(
        getInfoFeedback("mentorSearchBookedNearest", {
      ...
      if (
        ...
        try {
          ...
          if (nearestSession?.id) {
            ...
            setMentorSearchMessage(
              getInfoFeedback("mentorSearchWaitlistAdded").message,
            );
            return;
          }
        } catch {
          setMentorSearchMessage(
            getErrorFeedback("mentorSearchWaitlistFailed").message,
          );
          return;
        }
      }
      setMentorSearchMessage(
        backendError || getErrorFeedback("mentorSearchBookFailed").message,
      );
    }
  };
      ...
      const filtered = prev.filter(
        (item) =>
          !(
            ...
            Number(item.minScore) === filter.minScore
          ),
      );
  ...
  const paymentEligibleBookings = useMemo(() => {
    ...
    return bookings
      .filter((booking) =>
        ["ACCEPTED", "COMPLETED", "RESCHEDULE_REQUESTED"].includes(
  ...
  useEffect(() => {
    setPaymentForm((prev) => {
      const hasCurrent = paymentEligibleBookings.some(
        (booking) => String(booking.id) === String(prev.bookingId),
      );
      if (hasCurrent) {
        return prev;
      }
      const firstEligibleId = paymentEligibleBookings[0]
        ? String(paymentEligibleBookings[0].id)
        : "";
  ...
  useEffect(() => {
    ...
    const suggestedAmount = Number(
      selectedPaymentBooking?.session?.priceAmount || 0,
    );
  ...
  const filteredSessions = useMemo(() => {
    ...
    return sessions.filter(
      (session) => toDateKey(session.startTime) === selectedSessionDate,
    );
  ...
  const relatedBookings = useMemo(() => {
    ...
    return bookings
      .filter(
        (booking) =>
          booking?.learner?.id === profile.id ||
          booking?.session?.mentor?.id === profile.id,
      )
      .sort(
        (a, b) =>
          new Date(a?.session?.startTime || 0) -
          new Date(b?.session?.startTime || 0),
      );
  }, [bookings, profile?.id]);
  ...
  const upcomingReminder = useMemo(() => {
    ...
    const eligible = relatedBookings
      .filter((booking) =>
        ["ACCEPTED", "PENDING", "RESCHEDULE_REQUESTED"].includes(
          String(booking.bookingStatus || ""),
        ),
      )
      .map((booking) => ({
        booking,
        startMs: new Date(booking?.session?.startTime || 0).getTime(),
      }))
      .filter(
        (item) =>
          Number.isFinite(item.startMs) &&
          item.startMs > now &&
          item.startMs <= next24Hours,
      )
    ...
    const urgency =
      totalMinutes <= 30 ? "urgent" : totalMinutes <= 180 ? "soon" : "normal";
    return {
  ...
  const recommendationQueries = useMemo(() => {
    ...
    const fromProfileLearning = userSkills
      .filter((skill) =>
        ["Beginner", "Intermediate"].includes(String(skill.level || "")),
      )
    ...
    const profileSkillSource =
      fromProfileLearning.length > 0
        ? fromProfileLearning
        : fromAllProfileSkills;
    const deduped = Array.from(
      new Set([...fromWatchlist, ...profileSkillSource]),
    );
  ...
  const discoverQuickSearchSkills = useMemo(() => {
    const catalogNames = skillsCatalog
      ...
      .slice(0, 12);
    return Array.from(
      new Set([...recommendationQueries, ...catalogNames]),
    ).slice(0, 10);
  }, [recommendationQueries, skillsCatalog]);

  const displayedMentorResults = useMemo(() => {
    const withMinScore = mentorSearchResults.filter(
      (item) => Number(item?.score || 0) >= Number(mentorMinScore || 0),
    );
    const sorted = [...withMinScore].sort((a, b) => {
      ...
      if (mentorSortBy === "completion") {
        return (
          Number(b?.completionRateScore || 0) -
          Number(a?.completionRateScore || 0)
        );
      }
  ...
  const hasMoreMentorResults =
    paginatedMentorResults.length < displayedMentorResults.length;
  ...
  useEffect(() => {
    ...
    try {
      ...
      setRecentSearches(
        Array.isArray(storedRecent) ? storedRecent.slice(0, 8) : [],
      );
      setSavedSearchFilters(
        Array.isArray(storedFilters) ? storedFilters.slice(0, 8) : [],
      );
    } catch {
      setRecentSearches([]);
      setSavedSearchFilters([]);
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      `discoverRecentSearches:${profile.id}`,
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      `discoverSavedFilters:${profile.id}`,
      JSON.stringify(savedSearchFilters.slice(0, 8)),
    );
  ...
  const mentorAnalytics = useMemo(() => {
    const mentorBookings = bookings.filter(
      (b) => b.session?.mentor?.id === profile?.id,
    );
    const mentorPayments = payments.filter(
      (p) => p.booking?.session?.mentor?.id === profile?.id,
    );
    const now = new Date();
    ...
    const completedBookings = mentorBookings.filter(
    ...
    const repeatLearners = Array.from(learnersCount.values()).filter(
      (count) => count > 1,
    ).length;
    ...
    const topSkillDemand =
      Array.from(skillDemand.entries()).sort((a, b) => b[1] - a[1])[0]?.[0] ||
      "n/a";

    return {
      weeklyEarnings,
      sessionsPosted: sessions.filter((s) => s.mentor?.id === profile?.id)
        .length,
    ...
    return bookings
      ...
      .map((booking) => ({
        ...
        note: String(
          booking?.notes ||
            booking?.message ||
            "A new learner is waiting for approval.",
        ).trim(),
  ...
  const mentorScheduleItems = useMemo(() => {
    ...
    return sessions
      .filter(
        (session) =>
          session.mentor?.id === profile?.id &&
          new Date(session.startTime || 0).getTime() >= now - 15 * 60 * 1000,
      )
      .sort(
        (a, b) =>
          new Date(a.startTime || 0).getTime() -
      ...
      .map((session) => ({
        ...
        dateLabel: new Intl.DateTimeFormat(undefined, {
          month: "short",
          day: "2-digit",
        }).format(new Date(session.startTime || Date.now())),
        title: String(session.title || "Upcoming Session").trim(),
        subtitle: session.sessionType
          ? `${session.sessionType} • ${session.capacity || "Live session"}`
          : "Live session",
        startTime: formatSessionDateTime(session.startTime),
        isLive:
          new Date(session.startTime || 0).getTime() <= now &&
          new Date(session.endTime || 0).getTime() >= now,
  ...
  const mentorProgressItems = useMemo(() => {
    ...
    return roadmaps
      .filter(
        (roadmap) =>
          roadmap?.booking?.session?.mentor?.id === profile?.id ||
          roadmap?.booking?.learner?.id === profile?.id,
      )
      .slice(0, 3)
      .map((roadmap) => ({
        id: roadmap.id,
        name: String(
          roadmap?.booking?.learner?.fullName ||
            roadmap?.booking?.session?.title ||
            "Student",
        ).trim(),
        course: String(
          roadmap.title ||
  ...
  useEffect(() => {
    ...
    if (profileQualityScore < 80) {
      suggestions.push(
        `Improve profile quality from ${profileQualityScore}% to 100% by completing all fields.`,
      );
    }
    if (userSkills.length < 3) {
      suggestions.push(
        "Add at least 3 skill tags to improve matching accuracy.",
      );
    }
    if (
      !String(profile?.aboutMe || "").trim() ||
      String(profile?.aboutMe || "").trim().length < 80
    ) {
      suggestions.push(
        "Write a stronger bio: include experience, teaching style, and target learner level.",
      );
    }
    if (isMentor) {
      const topSkill = userSkills[0]?.name || "your top skill";
      suggestions.push(
        `Suggested session titles: "${topSkill} for Beginners", "${topSkill} Interview Prep", "Hands-on ${topSkill} Project".`,
      );
      const avgReleased = payments
        .filter(
          (p) =>
            p.booking?.session?.mentor?.id === profile.id &&
            p.status === "RELEASED",
        )
        .reduce(
          (sum, p, _, arr) =>
            sum + Number(p.amount || 0) / Math.max(arr.length, 1),
          0,
        );
      if (avgReleased > 0) {
        suggestions.push(
          `Current average released payment is INR ${avgReleased.toFixed(2)}. Suggested price band: INR ${(avgReleased * 0.9).toFixed(0)} - ${(avgReleased * 1.15).toFixed(0)}.`,
        );
      } else {
        suggestions.push(
          "Set an introductory price and increase after first 5 completed sessions.",
        );
      }
    }
  ...
  useEffect(() => {
    ...
    const loadRecommendations = async () => {
      ...
      try {
        const queries =
          recommendationQueries.length > 0 ? recommendationQueries : [""];
        const results = await Promise.all(
          queries.map((query) =>
            client.get("/api/v1/search/mentors", {
              params: { keyword: query, size: 12 },
            }),
          ),
        );
        ...
        results.forEach((response, idx) => {
          ...
          list.forEach((item) => {
            ...
            const totalScore =
              existing.aggregateScore * existing.occurrences +
              Number(item.score || 0);
            merged.set(item.mentorId, {
              ...existing,
        ...
        const sorted = Array.from(merged.values())
          .sort(
            (a, b) =>
              Number(b.aggregateScore || 0) - Number(a.aggregateScore || 0),
          )
          .slice(0, 8);
        ...
        if (sorted.length === 0) {
          setRecommendationMessage(
            "No recommendations yet. Add learning skills in your watchlist to get better matches.",
          );
        }
      } catch {
        if (isMounted) {
          setRecommendedMentors([]);
          setRecommendationMessage(
            getErrorFeedback("recommendationsLoadFailed").message,
          );
        }
      } finally {
        if (isMounted) {
        ...
        <aside className="fixed left-0 top-20 hidden h-[calc(100vh-5rem)] w-64 flex-col border-r border-slate-200/50 bg-slate-50 pt-6 md:flex dark:border-slate-800/50 dark:bg-slate-950">
          <div className="mb-8 px-6">
            <div className="flex items-center gap-3">
              <span className="w-7 h-7 rounded-lg premium-gradient text-white text-[9px] font-extrabold tracking-tight flex items-center justify-center">
                SS
              </span>
              <h2 className="font-headline text-lg font-extrabold text-emerald-900 dark:text-emerald-100">
                Skill Swapper
              </h2>
            </div>
          ...
          <nav className="flex-1 space-y-1">
            <Link
              className="flex items-center gap-3 rounded-r-full bg-emerald-50 px-6 py-3 font-bold text-emerald-900 dark:bg-emerald-900/30 dark:text-emerald-100"
              to="/mentors"
            >
            ...
            <Link
              className="flex items-center gap-3 px-6 py-3 text-slate-600 transition-colors hover:bg-emerald-50/50 hover:text-emerald-700 dark:text-slate-400 dark:hover:bg-emerald-900/10"
              to="/sessions"
            >
        ...
        <main className="px-4 pb-12 pt-6 md:ml-64 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-7xl">
            <header className="mb-10 flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <h1 className="mb-2 text-4xl font-extrabold tracking-tight text-on-surface">
                  Mentor Dashboard
                </h1>
                <p className="max-w-2xl text-on-surface-variant">
                  Welcome back to your curated teaching workspace. You have{" "}
                  {mentorSessionRequests.length} new session requests and a
                  workshop starting soon.
                </p>
              </div>
              <div className="hidden text-right lg:block">
                <span className="block text-[10px] font-bold uppercase tracking-widest text-on-tertiary-fixed-variant">
                  Expertise Level
                </span>
            ...
            <section className="mb-12 grid grid-cols-1 gap-6 md:grid-cols-3">
              ...
              <div className="rounded-xl border border-outline-variant/10 bg-surface-container-lowest p-6 shadow-sm">
                <div className="mb-4 flex items-start justify-between">
                  ...
                  <span className="rounded-full bg-tertiary-fixed/30 px-2 py-1 text-xs font-bold text-tertiary">
                    Monthly
                  </span>
                </div>
                <p className="mb-1 text-sm font-medium text-on-surface-variant">
              ...
              <div className="rounded-xl border border-outline-variant/10 bg-surface-container-lowest p-6 shadow-sm">
                <div className="mb-4 flex items-start justify-between">
                  ...
                  <span className="rounded-full bg-secondary-fixed/50 px-2 py-1 text-xs font-bold text-secondary">
                    Top 1%
                  </span>
                </div>
                <p className="mb-1 text-sm font-medium text-on-surface-variant">
              ...
              <section className="lg:col-span-1">
                ...
                <div className="space-y-4">
                  {mentorSessionRequests.length === 0 ? (
                    <div className="rounded-xl border border-outline-variant/10 bg-surface-container-lowest p-5 text-sm text-on-surface-variant">
                    ...
                    mentorSessionRequests.map((request) => (
                      <div
                        key={request.id}
                        className="rounded-xl border border-outline-variant/10 bg-surface-container-lowest p-5 transition-shadow hover:shadow-md"
                      >
              ...
              <section className="lg:col-span-2">
                <div className="rounded-2xl bg-surface-container-low p-6 sm:p-8 h-full">
                  <div className="mb-8 flex items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2">
                      <button
                        className="rounded-lg bg-surface-container-lowest p-2 text-on-surface shadow-sm"
                        type="button"
                        onClick={() => setSelectedSessionDate("")}
                        aria-label="Show all upcoming sessions"
                      >
                      ...
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          window.location.assign("/teach?tab=sessions")
                        }
                      ...
                      <div className="flex items-center gap-6">
                        <div className="flex h-16 w-16 flex-col items-center justify-center rounded-xl bg-primary font-headline text-on-primary">
                          <span className="text-xs font-bold uppercase">
                            {mentorScheduleItems[0].dateLabel.split(" ")[0]}
                          </span>
                          <span className="text-xl font-extrabold">
                            {mentorScheduleItems[0].dateLabel.split(" ")[1]}
                          </span>
                        </div>
                        <div>
                          <div className="mb-1 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-primary">
                            <span className="h-2 w-2 animate-pulse rounded-full bg-primary" />
                            {mentorScheduleItems[0].isLive
                              ? "Happening Now"
                              : "Next Up"}
                          </div>
                          <h3 className="text-xl font-bold text-on-surface">
                            {mentorScheduleItems[0].title}
                          </h3>
                          <p className="flex items-center gap-2 text-sm text-on-surface-variant">
                            <span className="material-symbols-outlined text-sm">
                              group
                            </span>{" "}
                            {mentorScheduleItems[0].subtitle}
                          </p>
                        </div>
                      </div>
                      <button
                        ...
                        onClick={() => {
                          if (mentorScheduleItems[0].meetingLink) {
                            window.open(
                              mentorScheduleItems[0].meetingLink,
                              "_blank",
                              "noopener,noreferrer",
                            );
                            return;
                          }
                          const feedback = showInfoFeedback({
                  ...
                  <div className="space-y-4">
                    {mentorScheduleItems.slice(1).map((session, index) => (
                      <div
                        key={session.id}
                        className="flex flex-col gap-3 rounded-xl border border-outline-variant/10 bg-surface-container-lowest p-4 md:flex-row md:items-center md:justify-between"
                      >
                        <div className="flex min-w-0 items-center gap-4">
                          <span className="w-16 text-xs font-bold text-on-surface-variant">
                            {session.startTime}
                          </span>
                          <div
                            className={`h-8 w-1 rounded-full ${index % 2 === 0 ? "bg-tertiary" : "bg-secondary"}`}
                          />
                        ...
                        <button
                          className="self-end rounded-full p-2 text-primary transition-colors hover:bg-primary/5 md:self-auto"
                          type="button"
                          onClick={() =>
                            window.location.assign(
                              `/teach?tab=sessions&sessionId=${encodeURIComponent(session.id)}`,
                            )
                          }
              ...
              <section>
                <div className="relative h-full overflow-hidden rounded-2xl bg-inverse-surface p-6 text-on-primary shadow-xl sm:p-8">
                  ...
                  <div className="relative z-10">
                    <h2 className="mb-8 text-xl font-bold text-inverse-on-surface">
                      Growth Analytics
                    </h2>
                    <div className="mb-8 grid grid-cols-2 gap-4">
                      <div className="rounded-xl border border-white/10 bg-white/5 p-4 backdrop-blur-md">
                        <span className="mb-1 block text-[10px] font-bold uppercase tracking-widest text-emerald-400">
                          Retention Rate
                        </span>
                        <span className="font-headline text-2xl font-bold">
                          {mentorAnalytics.completedBookings > 0
                            ? Math.min(
                                99.9,
                                (mentorAnalytics.repeatLearners /
                                  Math.max(
                                    mentorAnalytics.completedBookings,
                                    1,
                                  )) *
                                  100 +
                                  90,
                      ...
                      <div className="rounded-xl border border-white/10 bg-white/5 p-4 backdrop-blur-md">
                        ...
                        <span className="font-headline text-2xl font-bold">
                          {mentorAnalytics.bookingsReceived}
                        </span>
                      </div>
                    </div>
                    <div className="space-y-4">
                    ...
                    <button
                      className="mt-6 w-full rounded-xl border border-white/10 bg-white/10 py-3 text-sm font-bold backdrop-blur-sm transition-colors hover:bg-white/20"
                      type="button"
      ...
      {!isLoading && (
        <>
          ...
          {(isSessionsPage || isDiscoverPage) && upcomingReminder && (
            <section
              className={`dashboard-card session-reminder-banner session-reminder-${upcomingReminder.urgency}`}
            >
              <div className="session-reminder-head">
                <h3>Upcoming session reminder</h3>
                <span className="session-reminder-chip">
                  {upcomingReminder.countdownText}
                </span>
              </div>
              <p>
                <strong>{upcomingReminder.booking?.session?.title}</strong>{" "}
                starts in {upcomingReminder.countdownText} (
                {formatSessionDateTime(
                  upcomingReminder.booking?.session?.startTime,
                )}{" "}
                {userTimezone})
              </p>
              ...
              <section className="dashboard-card mentor-teaching-hero">
                <div className="mentor-teaching-hero-grid">
                  <div className="mentor-teaching-copy">
                    ...
                    <h3>
                      Design lessons, manage bookings, and keep your classroom
                      moving.
                    </h3>
                    <p className="mentor-teaching-description">
                      A single workspace for lesson strategy, session
                      throughput, learner trust, and content you can ship fast.
                    </p>
                    <div className="mentor-teaching-actions">
                      <button type="button" onClick={() => navigate("/teach")}>
                  ...
                  <div className="mentor-teaching-metrics">
                    <div className="mentor-metric-tile">
                      <span className="mentor-metric-label">
                        Weekly earnings
                      </span>
                      <strong>
                        INR {mentorAnalytics.weeklyEarnings.toFixed(2)}
                      </strong>
                    ...
                    <div className="mentor-metric-tile">
                      ...
                      <p>
                        Requests currently flowing into your teaching funnel.
                      </p>
                    </div>
                    <div className="mentor-metric-tile">
                      <span className="mentor-metric-label">
                      ...
                      <p>
                        {profileQualityScore >= 80
                          ? "Ready for higher conversion."
                          : "Improve bio and links to increase trust."}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {isMentor ? (
                  <ul>
                    <li>
                      Active sessions:{" "}
                      {
                        sessions.filter((s) => s.mentor?.id === profile?.id)
                          .length
                      }
                    </li>
                    <li>
                    ...
                    <li>
                      Verification tasks:{" "}
                      {
                        verificationTasks.filter(
                          (t) => t.mentor?.id === profile?.id,
                        ).length
                      }
                    </li>
                  </ul>
                  ...
                  <>
                    <ul>
                      <li>
                        Your bookings:{" "}
                        {
                          bookings.filter((b) => b.learner?.id === profile?.id)
                            .length
                        }
                      </li>
                ...
                {isDiscoverPage && (
                  <>
                    ...
                    {recommendationQueries.length > 0 ? (
                      <p className="muted">
                        Based on: {recommendationQueries.join(", ")}
                      </p>
                    ) : (
                      <p className="muted">
                        Add skills to your watchlist to get personalized mentor
                        matches.
                      </p>
                    )}
                    ...
                    {recommendationLoading ? (
                      ...
                      <ul>
                        {recommendedMentors.slice(0, 4).map((mentor) => {
                          const alreadySaved = savedMentors.some(
                            (saved) => saved.mentor?.id === mentor.mentorId,
                          );
                          return (
                            <li key={mentor.mentorId}>
                              <div>
                                <strong>{mentor.mentorName}</strong> | Match
                                score: {mentor.score}
                              </div>
                              <div className="muted">
                                Matched:{" "}
                                {(mentor.hitSkills || []).join(", ") ||
                                  "General relevance"}
                              </div>
                              <div className="inline-actions">
                                ...
                                {alreadySaved ? (
                                  <button
                                    type="button"
                                    onClick={() =>
                                      handleRemoveMentor(mentor.mentorId)
                                    }
                                  >
                                    Remove
                                  </button>
                                  ...
                                  <button
                                    type="button"
                                    onClick={() =>
                                      handleSaveMentor(mentor.mentorId)
                                    }
                                  >
                                    Save
                                  </button>
            ...
            <section className="dashboard-card">
              ...
              <form onSubmit={handleSubmitMentorVerification}>
                <input
                  ...
                  onChange={(e) =>
                    setMentorVerificationForm((prev) => ({
                      ...prev,
              ...
              {verificationWorkflowMessage && (
                <p className="muted">{verificationWorkflowMessage}</p>
              )}
              <ul>
                {mentorVerificationRequests.map((item) => (
                  <li key={item.id}>
                    Request #{item.id} | {item.documentType} |{" "}
                    <strong>{item.status}</strong>
                    {item.adminNote && (
                      <div className="muted">Admin note: {item.adminNote}</div>
                    )}
                  </li>
                ))}
              </ul>
              ...
              <form onSubmit={handleSubmitLearnerReview}>
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const selected = mentorCompletedBookings.find(
                ...
                >
                  ...
                  {mentorCompletedBookings.map((booking) => (
                    <option key={booking.id} value={booking.id}>
                      #{booking.id} - {booking.learner?.fullName} -{" "}
                ...
                <button
                  type="submit"
                  disabled={
                    !mentorLearnerReviewForm.bookingId ||
                    !mentorLearnerReviewForm.learnerId
                  }
                >
                  Submit learner review
                </button>
              </form>
            </section>
          )}
                  ...
                  {notifications.slice(0, 15).map((item) => (
                    <li key={item.id}>
                      <div>
                        ...
                        {!item.read && <span className="muted">(new)</span>}
                      </div>
          ...
          {isMentor && isMentorsPage && (
            <section className="dashboard-card">
              ...
              <ul>
                <li>
                  {t(language, "weeklyEarnings")}: INR{" "}
                  {mentorAnalytics.weeklyEarnings.toFixed(2)}
                </li>
                <li>
                  {t(language, "conversionFunnel")}:{" "}
                  {mentorAnalytics.sessionsPosted} sessions {"->"}{" "}
                  {mentorAnalytics.bookingsReceived} bookings {"->"}{" "}
                  {mentorAnalytics.completedBookings} completed
                </li>
                <li>
                  {t(language, "repeatLearners")}:{" "}
                  {mentorAnalytics.repeatLearners}
                </li>
                <li>
                  {t(language, "topSkillDemand")}:{" "}
                  {mentorAnalytics.topSkillDemand}
                </li>
              </ul>
            </section>
          )}
          ...
          {(isWalletPage || isDiscoverPage) && (
            <section className="dashboard-card">
              ...
              {!referralData ? (
                ...
                <>
                  <p>
                    <strong>{t(language, "yourReferralCode")}:</strong>{" "}
                    <span className="ref-code">{referralData.code}</span>
                  </p>
                  <ul>
                    <li>
                      {t(language, "invitedFriends")}:{" "}
                      {referralData.invitedFriends}
                    </li>
                    <li>
                      {t(language, "successfulReferrals")}:{" "}
                      {referralData.successfulReferrals}
                    </li>
                    <li>
                      {t(language, "creditsEarned")}:{" "}
                      {referralData.creditsEarned}
                    </li>
                  </ul>
                  <button type="button" onClick={recordReferralSuccess}>
                    {t(language, "addReferralSuccess")}
                  </button>
                </>
              )}
            </section>
            ...
            <section className="dashboard-card">
              ...
              <SearchFilters
                onSearch={(value) => {
                  ...
                  if (value.trim()) {
                    searchDebounceRef.current = setTimeout(() => {
                      runMentorSearch(value, activeFilters);
                    }, 300);
                  } else {
                    setMentorSearchResults([]);
                    setMentorSearchMessage(
                      getInfoFeedback("mentorSearchEnterKeyword").message,
                    );
                  }
                }}
                onFilterChange={(filter) => {
                  ...
                  setMentorMinScore(
                    nextRating > 0 ? Math.max(0, nextRating * 20) : 0,
                  );
                  setMentorSortBy(nextRating > 0 ? "rating" : "score");
                  runMentorSearch(searchQuery, filter);
                }}
              />
              ...
              <div className="inline-form">
                <select
                  value={mentorSortBy}
                  onChange={(e) => setMentorSortBy(e.target.value)}
                >
                ...
                />
                <button type="button" onClick={handleSaveCurrentFilter}>
                  Save current filter
                </button>
              </div>
                  ...
                  {discoverQuickSearchSkills.map((skill) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        runMentorSearch(skill, activeFilters);
                      }}
                    >
                      {skill}
                    </button>
                  ...
                  <div className="quick-search-chips">
                    {recentSearches.map((item) => (
                      <button
                        ...
                        onClick={() => {
                          setSearchQuery(item);
                          runMentorSearch(item, activeFilters, {
                            skipRecent: true,
                          });
                        }}
                      >
                ...
                <div>
                  ...
                  <ul className="saved-filters-list">
                    {savedSearchFilters.map((filter) => (
                      <li key={filter.id}>
                        <span>
                          Query: {filter.query || "All"} | Sort: {filter.sortBy}{" "}
              ...
              {mentorSearchLoading && (
                <p className="muted">Searching mentors...</p>
              )}
              {mentorSearchMessage && (
                <p className="muted">{mentorSearchMessage}</p>
              )}

              {paginatedMentorResults.length === 0 ? (
                <p className="muted">
                  No search results yet. Try a query like Java, React, DSA.
                </p>
              ) : (
                <ul>
                  {paginatedMentorResults.map((result) => (
                    <li key={result.mentorId}>
                      <strong>{result.mentorName}</strong> | Score:{" "}
                      {result.score}
                      <div className="muted">
                        Skill match: {result.skillMatch}, Rating:{" "}
                        {result.ratingScore}, Response:{" "}
                        {result.responseTimeScore}, Completion:{" "}
                      ...
                      <div className="inline-actions">
                        <a
                          ...
                          onClick={(e) => {
                            e.preventDefault();
                            requireLoginAndGo(`/mentors/${result.mentorId}`);
                          }}
                        >
                          Open public profile
                        </a>
                        <button
                          type="button"
                          onClick={() =>
                            handleBookNearestSession(result.mentorId)
                          }
                        >
                          Book now
                        </button>
                        <button
                          ...
                          onClick={() => {
                            ...
                            runMentorSearch(bestSkill, activeFilters);
                          }}
                        >
                          Find similar
                        </button>
              ...
              {hasMoreMentorResults && (
                <div
                  ...
                  className="discover-load-more-anchor"
                >
                  <button
                    type="button"
                    onClick={() =>
          ...
          {isMentorsPage && (
            <section className="dashboard-card">
              ...
              <div className="profile-quality-meter">
                ...
                <div className="profile-quality-track">
                  <div
                    ...
                    style={{ width: `${profileQualityScore}%` }}
                  />
                </div>
              </div>
              <p>
              ...
              <p>
                ...
                {profile?.walletAddress || "Not linked"}
              </p>
              <p>
              ...
              <p>
                <strong>About:</strong> {profile?.aboutMe || "Not set"}
              </p>
              <p>
                <strong>Verified skills:</strong>{" "}
                {profile?.verifiedSkills || "None yet"}
              </p>
              <p>
          ...
          {isMentorsPage && (
            <section className="dashboard-card">
              ...
              {userSkills.length > 0 ? (
                ...
                <p className="muted">
                  No personal skills found. Update your profile to add them.
                </p>
              )}

              <p>
                <strong>Platform skills:</strong>
              </p>
          ...
          {isDiscoverPage && (
            <section className="dashboard-card">
              ...
              <div className="inline-form">
                <select
                  value={overlapMentorId}
                  onChange={(e) => setOverlapMentorId(e.target.value)}
                >
                ...
                <button
                  type="button"
                  onClick={() =>
                    overlapMentorId && handleSaveMentor(Number(overlapMentorId))
                  }
                >
                  Save Mentor
                </button>
              ...
              <div className="inline-form">
                ...
                />
                <button type="button" onClick={handleAddWatchSkill}>
                  Add Skill
                </button>
              </div>
                  ...
                  {savedMentors.map((item) => (
                    <li key={item.id}>
                      ...
                      {item.mentor?.id && (
                        <a
                          className="meeting-link"
                          href={`/mentors/${item.mentor.id}`}
                          onClick={(e) => {
                            e.preventDefault();
                            requireLoginAndGo(`/mentors/${item.mentor.id}`);
                          }}
                        >
                          View profile
                        </a>
                      ...
                      <button
                        type="button"
                        onClick={() => handleRemoveMentor(item.mentor?.id)}
                      >
                        Remove
                      </button>
                    </li>
                ...
                <ul>
                  {watchedSkills.map((item) => (
                    <li key={item.id}>
                      ...
                      <button
                        type="button"
                        onClick={() => handleRemoveWatchSkill(item.skillName)}
                      >
                        Remove
                      </button>
                    </li>
          ...
          {(isSessionsPage || isDiscoverPage) && (
            <section className="dashboard-card">
              ...
              <div className="inline-form">
                ...
                <button type="button" onClick={handleFindOverlapSlots}>
                  Find overlap
                </button>
                <button type="button" onClick={handleRecommendOverlapSlots}>
                  Smart recommend
                </button>
              </div>

              {availabilityMessage && (
                <p className="muted">{availabilityMessage}</p>
              )}
              ...
              <p>
                <strong>Overlap slots in your timezone:</strong>
              </p>
              ...
              <p>
                <strong>Recommended slots (history + overlap):</strong>
              </p>
              {recommendedOverlapSlots.length === 0 ? (
                ...
                <ul>
                  {recommendedOverlapSlots.map((slot, idx) => (
                    <li
                      key={`recommended-${slot.dayOfWeek}-${slot.startTime}-${idx}`}
                    >
                      Day {slot.dayOfWeek}: {slot.startTime} - {slot.endTime} (
                      {slot.timezone})
                      <div className="muted">
                        Score: {slot.historyScore} | {slot.reason}
                      </div>
                    </li>
                  ))}
                </ul>
            ...
            <section className="dashboard-card">
              ...
              <div className="inline-form">
                ...
                <span className="muted">
                  Enter user id and tab out to block.
                </span>
              </div>

              <p>
                <strong>Your report status timeline:</strong>
              </p>
              {myReports.length === 0 ? (
                ...
                <ul>
                  {myReports.map((report) => (
                    <li key={report.id}>
                      Report #{report.id} | Target {report.targetType} #
                      {report.targetId || "-"} |{" "}
                      <strong>{report.status}</strong>
                      {report.escalated && (
                        <div className="muted">
                          Escalated level {report.escalationLevel || 1}:{" "}
                          {report.escalationReason || "No reason provided"}
                        </div>
                      )}
                    </li>
                  ))}
              ...
              {profile?.role === "ADMIN" && (
                <>
                  <p>
                    <strong>Moderation queue (open reports):</strong>
                  </p>
                    ...
                    <ul>
                      {moderationReports.map((report) => (
                        <li key={`moderation-${report.id}`}>
                          Report #{report.id} by {report.reporter?.fullName} on
                          user #{report.reported?.id} | {report.status}
                          <div className="inline-actions">
                            <button
                              type="button"
                  ...
                  <p>
                    <strong>Mentor verification moderation queue:</strong>
                  </p>
                  {mentorVerificationQueue.length === 0 ? (
                    <p className="muted">
                      No pending mentor verification requests.
                    </p>
                  ) : (
                    <ul>
                      {mentorVerificationQueue.map((request) => (
                        <li key={`verify-${request.id}`}>
                          Request #{request.id} by {request.mentor?.fullName} |{" "}
                          ...
                          <div className="inline-actions">
                            <button
                              type="button"
                              onClick={() =>
                                handleReviewMentorVerification(
                                  request.id,
                                  "APPROVED",
                                )
                              }
            ...
            <section className="dashboard-card mentors-search-surface">
              <div className="mentor-search-hero">
                <div>
                  ...
                  <h3>
                    Search mentors the same way you browse the learning hub.
                  </h3>
                  <p className="muted">
                    Query, sort, and score filters stay in the URL so the view
                    is shareable and persistent.
                  </p>
                </div>
                <div className="mentor-search-stats">
                  ...
                  <div>
                    <strong>
                      {mentorSearchLoading ? "..." : mentorSortBy}
                    </strong>
              ...
              <div className="mentor-search-toolbar">
                ...
                <div className="mentor-search-helper-row">
                  ...
                  <div className="mentor-search-links">
                    <Link className="meeting-link" to="/sessions">
                      Compare learning hub
                    </Link>
                    <button
                      type="button"
          ...
          {(isMentorsPage || isDiscoverPage) && (
            <section className="dashboard-card">
              ...
              {isMentor ? (
                <form onSubmit={handlePortfolioSave}>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setPortfolioForm((prev) => ({
                        ...prev,
                        pastTeachingSessions: e.target.value,
                      }))
                    }
                    rows={3}
                  />
                ...
                <p className="muted">
                  Portfolio editing is available on mentor accounts.
                </p>
              )}
            ...
            <section className="dashboard-card">
              ...
              <ul>
                {sessionPackages.map((pkg) => (
                  <li key={pkg.id}>
                    <strong>{pkg.title}</strong> - {pkg.sessionCount} sessions -{" "}
                    {pkg.discountPercent}% off - INR {pkg.totalPrice}
                  </li>
                ))}
              </ul>
            </section>
          ...
          {isSessionsPage && (
            <section className="dashboard-card">
              ...
              {roadmaps.length === 0 ? (
                <p className="muted">
                  No roadmaps yet. A roadmap is auto-created when a learner
                  books a session.
                </p>
              ) : (
                <ul>
                  {roadmaps.map((roadmap) => (
                    <li key={roadmap.id}>
                      ...
                      <div className="muted">
                        Booking #{roadmap.booking?.id} | Progress:{" "}
                        {roadmap.progressPercent}%
                      </div>
                      <textarea
                        value={roadmap.milestones || ""}
                        readOnly
                        rows={4}
                      />
                      <div className="inline-form">
                        <input
                          ...
                          onBlur={(e) =>
                            handleRoadmapUpdate(roadmap.id, e.target.value)
                          }
                        />
          ...
          {isSessionsPage && (
            <section className="dashboard-card">
              ...
              {verificationMessage && (
                <p className="muted">{verificationMessage}</p>
              )}
                  ...
                  <li key={task.id}>
                    ...
                    {!isMentor && (
                      <div className="inline-form">
                        ...
                        <button
                          type="button"
                          onClick={() => handleVerificationSubmit(task.id)}
                        >
                          Submit
                        </button>
                      </div>
                  ...
                  <ul>
                    {verificationSubmissions.map((submission) => (
                      <li key={submission.id}>
                        <div>
                          <strong>{submission.task?.title}</strong> by{" "}
                          {submission.learner?.fullName} - {submission.status}
                        </div>
            ...
            <section className="dashboard-card">
              ...
              <div className="session-calendar-row">
                {sessionCalendarDays.slice(0, 14).map((item) => (
                  <button
                    ...
                    type="button"
                    className={
                      selectedSessionDate === item.dateKey
                        ? "session-day-pill active"
                        : "session-day-pill"
                    }
              ...
              <ul>
                {filteredSessions.map((s) => (
                  <li key={s.id}>
                    <div>
                      <strong>{s.title}</strong> | {s.sessionType} | INR{" "}
                      {s.priceAmount}
                    </div>
                    <div className="muted">
                      {formatSessionDateTime(s.startTime)} to{" "}
                      {formatSessionDateTime(s.endTime)} ({userTimezone})
                    </div>
                    {s.meetingLink ? (
                      <a
                        className="meeting-link"
                    ...
                    {profile?.id === s.mentor?.id && (
                      <div className="inline-form">
                        ...
                        <button
                          type="button"
                          onClick={() => handleMeetingLinkSave(s.id)}
                        >
                          Save Link
                        </button>
                      </div>
          ...
          {(isSessionsPage || isDiscoverPage) && (
            <section className="dashboard-card">
              ...
              {bookingActionMessage && (
                <p className="muted">{bookingActionMessage}</p>
              )}
              {relatedBookings.length === 0 ? (
                ...
                <ul>
                  {relatedBookings.map((booking) => {
                    const isLearnerBooking =
                      booking?.learner?.id === profile?.id;
                    const isMentorBooking =
                      booking?.session?.mentor?.id === profile?.id;
                    return (
                      <li key={booking.id}>
                        <div>
                        ...
                        <div className="muted">
                          {formatSessionDateTime(booking.session?.startTime)} to{" "}
                          {formatSessionDateTime(booking.session?.endTime)} (
                          {userTimezone})
                        </div>
                        <div className="inline-actions">
                          {isLearnerBooking &&
                                ...
                                <button
                                  ...
                                  onClick={() =>
                                    handleBookingStatusUpdate(
                                      booking.id,
                                      "CANCELLED",
                                      "Reschedule request rejected and booking cancelled.",
                                    )
                                  }
                                >
                                  Reject request
            ...
            <section className="dashboard-card">
                ...
                >
                  ...
                  {paymentEligibleBookings.map((booking) => (
                    <option key={booking.id} value={booking.id}>
                      ...
                      {booking.bookingStatus} - INR{" "}
                      {booking.session?.priceAmount}
                    </option>
                  ))}
                </select>
                {paymentEligibleBookings.length === 0 && (
                  <p className="muted">
                    No payable bookings available. Accept a booking first or
                    complete a session without existing payment.
                  </p>
                )}
                <input
                  name="amount"
              ...
              <ul>
                {payments.map((payment) => (
                  <li key={payment.id}>
                    <div>
                      Booking #{payment.booking?.id} | INR {payment.amount} |{" "}
                      {payment.mode} | <strong>{payment.status}</strong>
                    </div>
                    <div className="inline-actions">
                      <button
                        type="button"
                        onClick={() =>
                          handleUpdatePaymentStatus(payment.id, "ESCROWED")
                        }
                      >
                        Escrowed
                      </button>
                      <button
                        type="button"
                        onClick={() =>
                          handleUpdatePaymentStatus(payment.id, "RELEASED")
                        }
                      >
                        Released
                      </button>
                      <button
                        type="button"
                        onClick={() =>
                          handleUpdatePaymentStatus(payment.id, "REFUNDED")
                        }
                      >
                        Refunded
                      </button>
          ...
          {(isSessionsPage || isDiscoverPage) && (
            <section className="dashboard-card">
              ...
              {selectedBooking && (
                <p className="muted">
                  Chat for booking #{selectedBooking.id}:{" "}
          ...
          <button className="mobile-logout submit-btn" onClick={onLogout}>
            Logout
          </button>
        </>
      )}
