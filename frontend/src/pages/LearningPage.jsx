...
export default function LearningPage({ notify }) {
  ...
  const normalizedRoadmaps = useMemo(() => {
    const mapped = roadmaps.map((roadmap, index) => ({
      ...
      mentorName: String(
        roadmap?.booking?.session?.mentor?.fullName || "Mentor",
      ).trim(),
      learnerName: String(
        roadmap?.booking?.learner?.fullName || "Learner",
      ).trim(),
      ...
      sessionTitle: String(
        roadmap?.booking?.session?.title || "Session",
      ).trim(),
  ...
  const lessonMilestones = useMemo(
    () =>
      parseMilestones(
        selectedRoadmap?.milestoneText ||
          selectedRoadmap?.milestones?.join("\n") ||
          "",
      ),
    [selectedRoadmap],
  );
  ...
  const averageProgress = useMemo(() => {
    ...
    return Math.round(
      normalizedRoadmaps.reduce(
        (sum, roadmap) => sum + Number(roadmap.progressPercent || 0),
        0,
      ) / normalizedRoadmaps.length,
    );
  }, [normalizedRoadmaps]);

  const completedCount = useMemo(
    () =>
      normalizedRoadmaps.filter((roadmap) => roadmap.progressPercent >= 100)
        .length,
    [normalizedRoadmaps],
  );
  const activeCount = useMemo(
    () =>
      normalizedRoadmaps.filter(
        (roadmap) =>
          roadmap.progressPercent > 0 && roadmap.progressPercent < 100,
      ).length,
    [normalizedRoadmaps],
  );
    ...
    const fromRoadmaps = normalizedRoadmaps
      ...
      .map((roadmap, index) => ({
        time: new Intl.DateTimeFormat(undefined, {
          hour: "2-digit",
          minute: "2-digit",
        }).format(new Date(roadmap.sessionStartTime)),
    ...
    <div className="min-h-screen bg-surface text-on-surface">
      <aside className="fixed left-0 top-20 z-20 hidden h-[calc(100vh-5rem)] w-72 flex-col border-r border-slate-200/60 bg-slate-50/95 pt-7 backdrop-blur-xl lg:flex">
        <div className="mb-8 px-6">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-black leading-tight text-emerald-900">
                The Curator
              </p>
          ...
          <p className="text-sm leading-relaxed text-slate-600">
            Stay focused on the path you’ve chosen. Follow modules, track your
            growth, and open each lesson in a detailed view.
          </p>
        </div>
      ...
      <main className="min-h-screen bg-surface px-4 pb-12 pt-6 sm:px-6 lg:ml-72 lg:px-8">
        <div className="mx-auto max-w-7xl space-y-10">
          <section
            id="overview"
            className="overflow-hidden rounded-[28px] bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 text-white shadow-[0_24px_60px_rgba(2,10,22,0.22)]"
          >
            <div className="relative grid gap-0 lg:grid-cols-[1.1fr_0.9fr]">
              <div className="relative p-6 sm:p-8 lg:p-12">
                ...
                <h1 className="max-w-2xl text-3xl font-black tracking-tight sm:text-4xl lg:text-5xl">
                  Build skills with a structured learning path, clear
                  milestones, and live mentorship.
                </h1>
                <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-300 sm:text-base">
                  Skill Swapper keeps your learning journey in one place. Real
                  roadmap progress loads from the backend, each card opens a
                  dedicated lesson view, and the sidebar now follows the section
                  you are reading.
                </p>
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <button
                    className="rounded-xl bg-white px-6 py-3 text-sm font-bold text-slate-950 shadow-lg transition-transform active:scale-95 hover:bg-emerald-50"
                    type="button"
                    onClick={() =>
                      document
                        .getElementById("path")
                  ...
                  <button
                    className="rounded-xl border border-white/15 bg-white/5 px-6 py-3 text-sm font-bold text-white backdrop-blur-md transition-transform active:scale-95 hover:bg-white/10"
                    type="button"
                    onClick={() =>
                      document
                  ...
                  <div className="mt-6 space-y-4">
                    <div className="rounded-2xl bg-white/8 p-4">
                      ...
                      <p className="mt-1 text-base font-bold text-white">
                        {selectedRoadmap?.mentorName ||
                          mentorHighlights[0]?.name ||
                          "Mentor"}
                      </p>
                    </div>
                    <div className="rounded-2xl bg-white/8 p-4">
                      ...
                      <p className="mt-1 text-base font-bold text-white">
                        {selectedRoadmap?.sessionTitle ||
                          "Open a lesson card below to view details"}
                      </p>
                    </div>
                    <div className="rounded-2xl bg-white/8 p-4">
                      <div className="mb-2 flex items-center justify-between text-sm font-semibold text-slate-300">
                        ...
                        <span>
                          {selectedRoadmap
                            ? `${selectedRoadmap.progressPercent}%`
                            : `${averageProgress}%`}
                        </span>
                      </div>
                      <div className="h-2 rounded-full bg-white/10 overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${selectedRoadmap ? selectedRoadmap.progressPercent : averageProgress}%`,
                          }}
                        />
                      </div>
                    </div>
          ...
          {selectedRoadmap && (
            <section
              id="lesson-detail"
              className="grid gap-6 rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm lg:grid-cols-[0.95fr_1.05fr] lg:p-8"
            >
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.24em] text-emerald-700">
                  Lesson Detail
                </p>
                <h2 className="mt-2 text-2xl font-black tracking-tight text-on-surface sm:text-3xl">
                  {selectedRoadmap.title}
                </h2>
                <p className="mt-4 text-sm leading-7 text-on-surface-variant">
                  Opened from the learning path cards. This page shows the live
                  roadmap pulled from the backend for booking #
                  {selectedRoadmap.bookingId || selectedRoadmap.id}.
                </p>
                <div className="mt-6 flex flex-wrap gap-3">
                  <button
                    className="rounded-xl bg-primary px-5 py-3 text-sm font-bold text-white shadow-sm transition-transform active:scale-95 hover:opacity-90"
                    type="button"
                  ...
                  <button
                    className="rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 transition-transform active:scale-95 hover:bg-slate-50"
                    type="button"
                    onClick={() =>
                      document
                        .getElementById("path")
              ...
              <div className="space-y-4 rounded-2xl bg-slate-50 p-5">
                ...
                <div className="rounded-2xl bg-white p-4">
                  ...
                  <p className="mt-1 text-xs text-on-surface-variant">
                    {formatSessionDateTime(selectedRoadmap.sessionStartTime)}
                    {selectedRoadmap.sessionEndTime
                      ? ` - ${formatSessionDateTime(selectedRoadmap.sessionEndTime)}`
                      : ""}
                  </p>
                </div>
                <div className="rounded-2xl bg-white p-4">
                  ...
                  <div className="h-2 overflow-hidden rounded-full bg-slate-100">
                    <div
                      ...
                      style={{ width: `${selectedRoadmap.progressPercent}%` }}
                    />
                  </div>
                </div>
                <div className="rounded-2xl bg-white p-4">
          ...
          <section id="path" className="space-y-6">
            <div className="flex items-end justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-black tracking-tight text-on-surface sm:text-3xl">
                  Your roadmap cards open full lesson views.
                </h2>
              </div>
              <span className="hidden text-sm font-bold text-slate-500 sm:inline-flex">
            ...
            {roadmapCards.length === 0 ? (
              <div className="rounded-2xl border border-slate-200 bg-white p-8 text-sm text-slate-600">
                No active roadmaps were found for your account. Book a session
                to start a roadmap or check your roadmap list.
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div
                          className={`flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br text-sm font-black text-white shadow-sm ${roadmap.fallbackColor || progressColor}`}
                        >
                          {String(index + 1).padStart(2, "0")}
                        </div>
                        <span
                          className={`rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em] ${roadmap.progressPercent >= 100 ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}
                        >
                      ...
                      <h3 className="mt-5 text-lg font-extrabold tracking-tight text-on-surface">
                        {roadmap.title}
                      </h3>
                      <p className="mt-1 text-sm text-on-surface-variant">
                        Mentor: {roadmap.mentorName}
                      </p>
                      <p className="mt-2 text-sm leading-7 text-on-surface-variant">
                        Learner progress is loaded from the backend and
          ...
          >
            <div className="space-y-6">
              <div className="flex items-end justify-between gap-4 flex-wrap">
                <div>
                  ...
                  <h2 className="mt-2 text-2xl font-black tracking-tight text-on-surface sm:text-3xl">
                    Live sessions and practice blocks.
                  </h2>
                </div>
                <button
                ...
                <div className="divide-y divide-slate-100 rounded-xl bg-white">
                  {upcomingSchedule.length === 0 ? (
                    <div className="p-6 text-sm text-slate-600">
                      No upcoming sessions are scheduled yet. Book a session to
                      see your learning schedule.
                    </div>
                  ) : (
            ...
            <aside className="space-y-6">
              ...
              <div className="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-6">
                ...
                <h3 className="mt-2 text-xl font-black tracking-tight text-on-surface">
                  Finish the next module and unlock your badge.
                </h3>
                <p className="mt-3 text-sm leading-7 text-slate-600">
                  Complete the roadmap, review the milestones, and open the
                  detailed lesson view to move step by step.
                </p>
              </div>
            </aside>
          </section>
          ...
          <section id="mentors" className="space-y-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-black tracking-tight text-on-surface sm:text-3xl">
                  People guiding the learning path.
                </h2>
              </div>
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {mentorsLoading ? (
                ...
                <div className="md:col-span-3 rounded-2xl border border-slate-200 bg-white p-6 text-sm text-slate-600">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <span>
                      No mentors found for this filter. Try another skill or
                      clear the filter.
                    </span>
                    <button
                      type="button"
                    ...
                    <div className="p-6">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          ...
                          <p
                            className={`mt-1 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-semibold ${mentor.liveNow ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-600"}`}
                          >
                            <span
                              className={`h-2 w-2 rounded-full ${mentor.liveNow ? "bg-emerald-500 animate-pulse" : "bg-slate-400"}`}
                            />
          ...
          >
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black tracking-tight text-on-surface sm:text-3xl">
                Proof of progress that can follow you anywhere.
              </h2>
              <p className="mt-4 text-sm leading-7 text-on-surface-variant">
                Track completed lessons, export credentials, and keep your
                learning progress visible as you move through the platform.
              </p>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                ...
                <p className="mt-2 text-sm text-on-surface-variant">
                  Mentor-reviewed project and portfolio badge.
                </p>
              </div>
            </div>
          </section>
      ...
      <aside
        className={`fixed left-0 top-20 z-50 flex h-[calc(100vh-5rem)] w-72 max-w-[85vw] flex-col border-r border-slate-200/60 bg-slate-50/95 pt-7 backdrop-blur-xl transition-transform duration-300 lg:hidden ${mobileSidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
