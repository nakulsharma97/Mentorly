...
public class AnalyticsService {
    ...
    private static final DateTimeFormatter WEEKDAY_FORMATTER = DateTimeFormatter.ofPattern("EEE", Locale.getDefault());
    ...
    private AnalyticsDataset loadDataset(User currentUser, int rangeDays) {
        ...
        List<Booking> filteredBookings = bookings.stream()
                .filter(booking -> eventTime(booking).isAfter(cutoff) || eventTime(booking).isEqual(cutoff))
                .toList();

        List<Payment> payments = mentor
                ? paymentRepository.findByBookingSessionMentorIdAndCreatedAtGreaterThanEqual(currentUser.getId(),
                        cutoff)
                : paymentRepository.findByBookingLearnerIdAndCreatedAtGreaterThanEqual(currentUser.getId(), cutoff);
    ...
    private AnalyticsDtos.AnalyticsSummaryDto buildSummaryFromDataset(AnalyticsDataset dataset) {
        ...
        List<Booking> completedBookings = relevantBookings.stream()
                .filter(booking -> "COMPLETED".equalsIgnoreCase(String.valueOf(booking.getBookingStatus())))
                .toList();
        ...
        int cancelledCount = (int) relevantBookings.stream()
                .filter(booking -> "CANCELLED".equalsIgnoreCase(String.valueOf(booking.getBookingStatus())))
                .count();
        ...
        int upcomingSessionsCount = (int) relevantBookings.stream()
                ...
                .filter(booking -> List.of("PENDING", "ACCEPTED", "RESCHEDULE_REQUESTED")
                        .contains(String.valueOf(booking.getBookingStatus()).toUpperCase(Locale.ROOT)))
                .count();
        ...
        int goalProgressPercent = (int) Math.min(100, Math.round((goalCurrent / Math.max(1, goalTarget)) * 100));
        ...
        amountPayments.forEach(payment -> {
            ...
            if (bookingId != null) {
                paymentByBookingId.put(bookingId, paymentByBookingId.getOrDefault(bookingId, 0.0) + amountOf(payment));
            }
        });
        ...
        double maxMonthValue = monthBuckets.stream().mapToDouble(AnalyticsDtos.TrendBucketDto::value).max().orElse(1);

        List<AnalyticsDtos.TrendBucketDto> weeklyBuckets = buildWeeklyTrend(relevantBookings);
        double maxWeeklyValue = weeklyBuckets.stream().mapToDouble(AnalyticsDtos.TrendBucketDto::value).max().orElse(1);
    ...
    private AnalyticsDtos.LearnerHistoryDto buildLearnerHistory(List<Booking> bookings, List<Payment> settledPayments) {
        ...
        List<AnalyticsDtos.MentorHistoryDto> mentors = history.values().stream()
                .sorted(Comparator
                        .comparingDouble((MutableMentorHistory row) -> row.totalSpend).reversed()
                        .thenComparingInt((MutableMentorHistory row) -> row.sessionsAttended).reversed()
        ...
        int totalSessionsAttended = mentors.stream().mapToInt(AnalyticsDtos.MentorHistoryDto::sessionsAttended).sum();
        double totalHoursAttended = mentors.stream().mapToDouble(AnalyticsDtos.MentorHistoryDto::totalHours).sum();
    ...
    private List<AnalyticsDtos.TrendBucketDto> buildMonthTrend(List<Payment> payments, OffsetDateTime now) {
        ...
        for (int i = 5; i >= 0; i -= 1) {
            months.add(now.minusMonths(i).withDayOfMonth(1).withHour(0).withMinute(0).withSecond(0).withNano(0));
        }
    ...
    private List<AnalyticsDtos.TrendBucketDto> buildWeeklyTrend(List<Booking> bookings) {
        ...
        days.forEach(day -> counts.put(day.getYear() + "-" + day.getMonthValue() + "-" + day.getDayOfMonth(), 0.0));
