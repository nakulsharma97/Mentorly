...
class BookingLifecycleServiceTest {
    ...
    void recordMentorJoinTransitionsScheduledToLive() {
        com.mentorly.session.LiveSessionStatus originalStatus = com.mentorly.session.LiveSessionStatus.SCHEDULED;
        ...
        assertEquals(com.mentorly.session.LiveSessionStatus.LIVE, session.getLiveSessionStatus());
    ...
    void initiateCompletionConfirmationsTransitionsLiveToEnded() {
        ...
        assertEquals(com.mentorly.session.LiveSessionStatus.ENDED, session.getLiveSessionStatus());
    ...
    void initiateCompletionConfirmationsDoesNotEndScheduledSession() {
        session.setLiveSessionStatus(com.mentorly.session.LiveSessionStatus.SCHEDULED);
        session.setEndTime(OffsetDateTime.now().minusHours(1)); // session already ended
        booking.setBookingStatus(BookingStatus.IN_PROGRESS);
        booking.setCompletionReviewStatus(CompletionReviewStatus.NOT_APPLICABLE);
        booking.setSession(session);
        ...
        assertEquals(1, initiated);
        assertEquals(com.mentorly.session.LiveSessionStatus.SCHEDULED, session.getLiveSessionStatus());
        verify(sessionRepository, never()).save(session);
    }
    ...
    void requestRescheduleTransitionsBookingAndSession() {
        ...
        assertEquals(com.mentorly.session.LiveSessionStatus.RESCHEDULED, session.getLiveSessionStatus());
